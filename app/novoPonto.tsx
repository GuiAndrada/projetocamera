...
export default function NovoPonto() {
  ...
  const salvarPonto = async () => {
    if (!nome.trim()) {
      Alert.alert('Atenção', 'Digite o nome do ponto de coleta!');
      return;
    }
    if (!localizacao) {
      Alert.alert('Atenção', 'Selecione a localização no mapa!');
      return;
    }

    try {
      const response = await fetch(`http://${IP}/app_teste/pontos_salvar.php`, {
        method: 'POST',
      ...
      if (data.status === 'sucesso') {
        Alert.alert('✅ Ponto cadastrado!', 'O ponto de coleta foi adicionado ao mapa!', [
          { text: 'OK', onPress: () => router.back() }
        ]);
      } else {
        Alert.alert('Erro', 'Falha ao salvar o ponto.');
      }
    } catch (e) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
      <Text style={styles.titulo}>📍 Novo Ponto de Coleta</Text>
      <Text style={styles.subtitulo}>Cadastre um local ideal para descarte de lixo eletrônico</Text>

      <Text style={styles.label}>Nome do local *</Text>
      <TextInput
        style={styles.input}
        placeholder="Ex: Ecoponto Central, Loja TechRecicla..."
        value={nome}
      ...
      <TextInput
        ...
        placeholder="Ex: Aceita celulares, baterias e computadores."
        value={descricao}
      ...
      >
        {localizacao && <Marker coordinate={localizacao} pinColor="#8e44ad" />}
      </MapView>
      <Text style={styles.dica}>💡 Toque no mapa para marcar o local exato</Text>

      <TouchableOpacity style={styles.botaoSalvar} onPress={salvarPonto}>
...
const styles = StyleSheet.create({
  ...
  titulo: { fontSize: 24, fontWeight: 'bold', color: '#8e44ad', marginBottom: 4 },
  ...
  botaoCamera: { backgroundColor: '#2980b9', padding: 12, borderRadius: 8, alignItems: 'center', marginBottom: 10 },
  botaoLoc: { backgroundColor: '#8e44ad', padding: 12, borderRadius: 8, alignItems: 'center', marginBottom: 10 },
  botaoSalvar: { backgroundColor: '#27ae60', padding: 14, borderRadius: 10, alignItems: 'center', marginTop: 20 },
